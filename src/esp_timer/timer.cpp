...
bool ESPTimer::clearItem(Type type, uint32_t id) {
	...
	switch (type) {
	case Type::Timeout:
		remove_by_id(timeouts_);
		break;
	case Type::Interval:
		remove_by_id(intervals_);
		break;
	case Type::Sec:
		remove_by_id(secs_);
		break;
...
ESPTimerStatus ESPTimer::getItemStatus(Type type, uint32_t id) {
	...
	switch (type) {
	case Type::Timeout:
		find_by_id(timeouts_);
		break;
	case Type::Interval:
		find_by_id(intervals_);
		break;
	case Type::Sec:
		find_by_id(secs_);
		break;
...
ESPTimerStatus ESPTimer::getStatus(uint32_t id) {
	...
	ESPTimerStatus s;
	s = getItemStatus(Type::Timeout, id);
	if (s != ESPTimerStatus::Invalid)
		return s;
	s = getItemStatus(Type::Interval, id);
	if (s != ESPTimerStatus::Invalid)
		return s;
	s = getItemStatus(Type::Sec, id);
	if (s != ESPTimerStatus::Invalid)
		return s;
	s = getItemStatus(Type::Ms, id);
	if (s != ESPTimerStatus::Invalid)
		return s;
	s = getItemStatus(Type::Min, id);
	if (s != ESPTimerStatus::Invalid)
		return s;
...
void ESPTimer::timeoutTask() {
	while (running_.load(std::memory_order_acquire)) {
		...
		TimerVector<std::function<void()>> toCall{
		    TimerAllocator<std::function<void()>>(usePSRAMBuffers_)
		};
		...
		while (it != timeouts_.end()) {
			...
			} else if (it->status == ESPTimerStatus::Stopped ||
	...
	while (running_.load(std::memory_order_acquire)) {
		...
		intervals_.erase(
		    std::remove_if(
		        intervals_.begin(),
		        intervals_.end(),
		        [](const IntervalItem &n) {
			        return n.status == ESPTimerStatus::Stopped ||
			               n.status == ESPTimerStatus::Completed;
		        }
		    ),
		    intervals_.end()
		);
